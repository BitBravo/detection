...
cocoSsd.load(
  {
    base: 'lite_mobilenet_v2', // Choose the lightest model
    modelUrl: 'models/official/model.json' // Use the local model
}
...
function predictWebcam() {
  ...
  model.detect(VIDEO).then(function(predictions) {
    ...
    for (let n = 0; n < predictions.length; n++) {
      ...
      if (predictions[n].score > MIN_DETECTION_CONFIDENCE && predictions[n].class === "person") {
